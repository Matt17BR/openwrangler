...
29. `sortRows`, `filterRows`, `dropMissingRows`, `dropDuplicates`, `selectColumns`, `dropColumns`, `renameColumn`, `cloneColumn`, `castColumn`, `formula`, `textLength`, `oneHotEncode`, `multiLabelBinarize`, `findReplace`, `stripText`, `splitText`, `capitalizeText`, `lowerText`, `upperText`, `minMaxScale`, `roundNumber`, `floorNumber`, `ceilNumber`, and `formatDatetime` accept only public column references shaped as `{id, name}` wherever they address an input column. Transform filters/sorts are a distinct stable-reference IR from name-addressed viewing queries. The runtime binds each reference against the exact input schema and lineage to a private `{id, name, position}` value before adapter dispatch; a transform filter's declared semantic type must also match that exact input schema. Persisted steps and protocol metadata never expose the private position. Unknown, stale, type/name-mismatched, repeated, or malformed identities, duplicate input-lineage IDs, output-name collisions, and private row-identity namespace collisions fail closed. There is no string/name fallback. Pandas live execution and generated code must address these operations positionally, including exact in-place replacement under duplicate and non-string labels; omitted or explicitly empty missing-row columns and omitted duplicate-row columns mean every visible input column and must exclude the private row identity. Generated categorical outputs and explicit new value columns must be appended without ambiguous label assignment and must pass collision/private-namespace guards. An edited latest step must retain its original step ID while deriving globally unique new-output IDs as `c:step:<stepId>:<created-output-ordinal>`. State-dependent reuse of the replaced result's identities is forbidden so replay publishes the same edited lineage.
