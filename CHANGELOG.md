...
- The optional Data Wrangler comparison now has a manual 1M × 100 mixed-type Parquet study. It uses five fresh
  ...
  compressed size and must be 400–640 MiB. Local runs stop above 12 GiB RSS or 64 processes, cap each attempt at five
  minutes and each invocation at one hour, and check power and CPU state around every session. Normal CI keeps the
  existing small regression benchmark.
