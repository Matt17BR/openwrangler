...
- Pandas and Polars compute single-precision float statistics in double precision, as DuckDB and R do. Means and sums match across engines, and Pandas no longer omits the standard deviation when single-precision arithmetic would overflow.
...
- Polars datetime conversion accepts date-only and space-separated text, turns invalid suffixes into null and refuses recognized timezone offsets instead of silently dropping them.
...
- Pandas native duration categories, Arrow duration categories and Arrow duration columns, including dictionaries, search displayed labels while retaining raw-text matches.
...
- Pandas durations retain their stored unit when preparing cells and choices, so valid values outside the nanosecond
  range no longer overflow during display.
...
- Installed Python dependencies remain available when unrelated files or subdirectories change in their parent directories.
...
- Generated Pandas Pivot Wider handles object identifiers with `NaT` and mixed scalar values like live execution.
  Generated integer helpers match live scalar admission and preserve Group By key values and dtypes.
