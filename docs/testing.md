...
- `npm run test:scripts:workflow` runs focused tests for CI path selection, required result handling, fixed action
  revisions, and release workflow fan-ins. It also runs the separately focused candidate and install-policy tests.
...
The selected `package-editor` job performs both TypeScript typechecks and the complete TypeScript unit/component
