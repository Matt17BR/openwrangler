...
- `npm run test:r-contract` sources the production R module, runs native R assertions, and then sends real base
  ...
  order. Filter Rows and Sort Rows tests cover stable source-row identities, current-view conversion, `NA` versus
  `NaN`, compound sort priority, stable ties, missing-value placement, history, diffs, and data-table key behavior.
  ...
  deadlines. CI owns this command in a focused R 4.4/4.5 matrix and overlaps it with portable contract tests after
  fast feedback passes.
...
Ready pull requests that change only the Data Wrangler comparison harness use the `Benchmark harness` CI job. The
classifier accepts an exact list of comparison scripts, their focused tests and fixtures, and the related performance
documentation. Fast feedback still runs, and the focused job adds the comparison and Linux PSS Node tests, the
notebook-trial Vitest test, `test_installed_editor_fixtures.py`, and `build:test-extension`. It does not run the
comparison itself. Adding any other path selects the normal full matrix. The macOS, Windows, and CodeQL workflows
still publish their required contexts through the existing no-work cells for this narrow lane.
...
Every required context has one primary evidence owner. [CI and release checks](ci.md) maps the pull-request, scheduled, and release tiers. On ready pull requests, `validate` runs after the main CI jobs and requires every selected result to have the expected outcome. The opt-in Remote SSH result must succeed when selected and be skipped otherwise. Cross-platform runtime and CodeQL remain separate required workflows because one GitHub Actions workflow cannot depend directly on another workflow's job results. Drafts use the separate `Draft feedback` name and do not publish the protected `validate` context.

Ready pull requests that change only `package-lock.json` run fast feedback, `npm ls`, a full `npm audit`, the TypeScript/webview unit suite, and the clean canonical build and VSIX verification. The one TypeScript contract that starts a real Pandas interpreter stays with the full Python-backed matrix. The focused path skips the other Python, R, visual, extension-host, and packaged-editor jobs. Adding `package.json` or any second changed path restores the complete pull-request matrix; main pushes and release checks are unchanged.
...
stable consumer against that artifact ID. Protected pull-request CI is the sole complete source-suite owner; stable
Linux runs only live repository/dependency/runtime checks and exact-artifact packaged-editor acceptance. The other
jobs add Cursor, installed performance, released and remote Jupyter, Remote SSH, and macOS/Windows compatibility.
With `publish: true`,
...
the single comprehensive owner of the Jupyter deny, complete allow, PySpark, and remote-kernel journeys. Cursor runs
only `candidate-compatibility-seam`, a real allow-path proof through Variables, renderer open, one page, close, and
zero-session cleanup. The profile deliberately omits both editors' generic restricted-workspace, Python-environment,
...
For affected pull requests, the released-Jupyter CI job downloads and revalidates the same checksum-bound VSIX produced
by `canonical-vsix`; it never rebuilds the extension. Documentation-only, package-only, and draft pull requests require
that conditional job to be skipped. The draft aggregate still fails deliberately; every ready full-matrix pull request
requires the job to succeed through the fail-closed `validate` aggregate. Protected-branch pushes run only `Fast
feedback`, so neither this job nor the aggregate runs after merge.
...
literate-document journeys in VS Code and Cursor; VS Code alone owns the complete value and categorical catalogs and
...
the focused literate invocation covers plain `.R`, `.Rmd`, and `.qmd` in both VS Code and Cursor. Windows skips that
document coverage. For local Windows files, the package manifest hides the explicit **Run R Document** Explorer and
...
current candidate gate repeats that journey in fresh VS Code and Cursor phases. The Python Quarto check accepts
...
commit `b521f52` measured the two-shard redesign at 21m57s versus run #73's 33m15s (34% lower). Its slower local-R
shard took 15m19s versus the former 31m15s serial lane (51% lower), while runner use rose from 119.75 to 130.78 minutes
(9.2%). macOS was the measured bottleneck at 19m54s. Every raw outcome except lifecycle core passed; core reached
`restart:start` before its outer process crossed the unchanged 300-second hard deadline. The fan-in failed,
publication was skipped, and no `v1.99.6` was created.

Preview release [run #75](https://github.com/Matt17BR/openwrangler/actions/runs/31834973654) from protected `main`
commit `917341a` finished in 21m18s and used 134.07 positive-duration runner-minutes. Its sole raw candidate blocker was
Linux lifecycle core: VS Code 1.133.0 crossed the outer deadline at about 300.012 seconds after
...
and immediately publishes any sealed failure evidence before its shard-local deferred raw-outcome check. Core,
native-frame, restart, terminal, and literate phases run both VS Code and Cursor; value and categorical phases run the
single comprehensive VS Code owner. Separately, generic macOS/Windows platform cells perform no R setup or native-R
tail. Parallel
...
manual protected-main dispatch, default `publish: false`, one package producer and canonical triple, one exact-artifact
Linux VS Code full packaged-editor owner, a focused Linux Cursor `platform-smoke`, and exactly one non-matrix caller of
the shared candidate workflow. That reusable workflow owns
fixed parallel generic-platform, `r_platform`, performance, Python, remote-R, and Linux local-R jobs.
Generic macOS/Windows platform cells must set `platform-smoke` for VS Code and must not prepare R, run a native-R tail,
or repeat the Cursor fork seam owned by the single pinned Linux smoke. No generic consumer other than Linux VS Code may run the full packaged journey. The `r_platform` matrix must contain the
macOS and Windows cells, prepare R once per cell, order VS Code-only `core-operations`, `native-frames`, then
`kernel-restart`, perform a fresh exact verifier and distinct immediate upload for each, and defer a literal-success
verdict for all three raw outcomes until all uploads have run. Local R is exactly two non-cancelling shard matrix cells: lifecycle orders
`core-operations` before `kernel-restart` before `interactive-terminal` before `literate-documents`, while editing
orders `native-frames` before `value-operations` before `categorical-operations`. Tests require setup once per shard,
...
serial diagnostic and is tested against that truthful contract rather than the candidate fan-out. Preview run #74
measured the prior two-shard graph at 21m57s overall versus #73's 33m15s (34% lower), and 15m19s for its slower R shard
versus 31m15s (51% lower); runner use rose from 119.75 to 130.78 minutes (9.2%), and macOS was the 19m54s bottleneck.
Its lifecycle core nevertheless exceeded the unchanged outer deadline after `restart:start`. Preview run #75 measured
