...
both engine seams. Accepted fixtures cover explicit imports, multiline control flow, and a closure with its own
`return`; each generated script executes and must match live output. Session tests reject incompatible code before
creating a new draft or replacing an applied step, with the revision, plan, committed frame, lineage, and draft state
unchanged.
