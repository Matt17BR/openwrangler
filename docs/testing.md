...
Set `OPEN_WRANGLER_REAL_DATA_WRANGLER=1` together with the released-Jupyter opt-in to add a coexistence gate against the exact Marketplace baseline `ms-toolsai.datawrangler@1.24.2`. The runner installs that package into a separate private extensions directory, verifies only its public extension ID/version and activation state, and never opens or inspects its package source or assets. VS Code receives two additional disposable user-data profiles. In each fresh profile, the runner selects the exact private released-Jupyter kernel and executes the dataframe cell first. That cell emits one bounded private `sys.executable` and PID marker before displaying the dataframe. The runner then acquires the exact selected kernel under a separate bound, uses its non-history execution API for a second identity reading without changing notebook cells, outputs, execution counts, or dirty state, and requires both identities and the later setup cell to match. The provider modal must remain unresolved while the notebook's first physical output is a Pandas dataframe rendered as ordinary HTML; no setup or formatter-preparation cell may run first. The Open Wrangler profile then selects **Use Open Wrangler** and requires that exact DOM owner, notebook output object, output items, and execution order to remain fixed while the existing output upgrades inline. A second execution, output replacement, notebook edit, session panel, or Open Wrangler MIME rewrite fails the phase. The Data Wrangler profile selects **Keep Data Wrangler** and requires the same first output to remain ordinary. Each profile is then relaunched, and the other two bounded phases prove the selected global preference survives, the modal is not repeated, and a newly started or restarted Python kernel contains Open Wrangler's MIME v2 output only for the Open Wrangler choice. The Data Wrangler choice requires the real extension to remain active beside a successful native Jupyter dataframe output while Open Wrangler's MIME and kernel-consent prompt stay absent. Compilation and unit tests do not count as this installed evidence. The gate may never depend on a copied implementation detail or a private MIME identifier. The proprietary extension is not launched in Cursor; the real-provider coexistence gate remains VS Code-only under its [Marketplace license](https://marketplace.visualstudio.com/items/ms-toolsai.datawrangler/license), while Open Wrangler's provider coordination remains covered independently in Cursor.
