...
fixed-metric batches, ten top-value actions, and three nonconstant numeric histogram actions. It therefore completes
in exactly 17 actions instead of 23. A separate fused-versus-single-column fixture compares both Python objects and
canonical compact UTF-8 bytes across wide integers, decimals, float null/NaN/infinity values, Unicode text, dates,
and timestamps. Column-value discovery has its own exact one-action regression.
