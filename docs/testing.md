...
Generic macOS/Windows platform cells must set `platform-smoke` for both VS Code and Cursor and must not prepare R or
run a native-R tail. No generic consumer other than Linux VS Code may run the full packaged journey. The `r_platform` matrix must contain the
