...
- `npm run test:scripts:portable` runs the remaining general or Linux-owned `scripts/*.test.mjs`, including GitHub
  ...
  isolated media tests below. Pull-request CI runs it in the package/editor job.
...
- `npm run test:scripts:media` runs the PNG-heavy README-media tests alone, with one test file and a 1 GiB V8 heap ceiling. Pixel mismatches report only the first differing coordinate/channel rather than asking Node to render a multi-million-byte assertion diff. The package/editor job reaches it through `test:scripts:portable`.
...
- `node scripts/run-extension-tests.mjs platform-smoke` uses the same isolated real-editor runner for the shorter Cross check. It opens one CSV through the public action, verifies activation, grid rendering, one sort, unchanged source bytes, and clean shutdown. It does not repeat the histogram, persistence, recovery, or context-menu journeys owned by the broader editor lanes.
...
`package.json`, `package-lock.json`, toolchain, workflow, classifier, validation, and lock-owner changes select all five
conditional behavior jobs. There is no narrow dependency-lock or release-infrastructure mode in the current PR workflow.
...
The JavaScript/TypeScript job owns both typechecks. The selected `package-editor` job runs TypeScript coverage,
performs one clean production build, packages and verifies the VSIX, and runs the stable extension-host
...
Coverage is a regression guard, not a substitute for scenario acceptance. TypeScript/webview floors are 60% statements, 55% branches, 60% functions, and 65% lines; Python runtime coverage must remain at or above 78%. The required pull-request coverage jobs provision Temurin Java 17, exact `pyspark[connect]==4.2.0`, and Pandas `>=2.2,<3.0` before enforcing those floors. CI prints the coverage result and retains no coverage artifact. Release candidates consume the accepted source and do not repeat source coverage.
...
The current PR workflow keeps source-level editor checks in the package/editor job.
That job builds and verifies one VSIX, then runs the extension-host journey from the checkout. The standalone
...
Protected-branch pushes run every CI lane; CodeQL retains its separate two-analyzer push check.
...
jobs. Dependabot configuration and documentation select no conditional job; Python-test-only changes select Python.
Package and lockfile changes select all consumers. The final `validate` job blocks missing, failed, cancelled, or
unexpected results while accepting intentional skips.
