...
- `npm run test:r-contract` sources the production R module, runs native R assertions, and then sends real base
  ...
  the exact ordered 31-operation live/generated catalog, and the dedicated TypeScript export contract binds the same
  bridge/public order to byte-exact clipboard and atomic `.R` saves for distinct executable operation-labelled
  buffers. Fresh installed all-31 candidate/performance evidence remains outstanding. A fresh
...
viewing-only. Pivot wider, transpose, explode, and unnest are not implied by this evidence.
...
buffer is copied and atomically saved byte for byte. Fresh hosted candidate and installed all-31/performance evidence
...
advertised operation registry. Dedicated local-source contracts own the strict ordered 31-operation live/generated
