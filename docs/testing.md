...
Generated fixtures prove exact fresh globals, no ambient NumPy, DuckDB helper, or plan-dependent Polars helper leakage,
cross-step namespace isolation, Pandas/Polars Series normalization, and scalar-result rejection. Terminal LF and
non-LF `splitlines()` separators must match the streaming generated-size model and preserve rejection before the 4 MiB
allocation cap. Session tests reject incompatible code before creating a new draft or replacing an applied step, with
the revision, plan, committed frame, lineage, and draft state unchanged.
