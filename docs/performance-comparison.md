...
and types. Every trial checks the fixture ID and SHA-256, stable file identity, and three sentinel cells. Before the
environment gate, the runner makes and certifies one byte-for-byte copy in that trial's private directory, then writes
the notebook that names it. A failed gate cleans up that exact copy. The original fixture is never passed to either
...
For the primary warm study, a private notebook loads the copied source into one named Pandas or Polars dataframe in an
untimed setup cell. The measured expression cell only evaluates that already-resident variable, so notebook preview
timing excludes CSV/Parquet parsing. Cold trials intentionally load the source in the measured cell, as described in
the sampling section. A separate diagnostic duration records source loading for warm trials but is not folded into
their preview comparison. Shape, exact Python type/module, fixture ID/hash, and three deterministic sentinels are
verified in an ordinary visible notebook cell before timing and again after the workbench closes. No local work data,
pickle, external network dataset, or user path is permitted.
