...
> Generated from `package.json` and `protocol/openwrangler.v4.schema.json`. Do not edit this file by hand; run `npm run generate:reference`.
...
| `openWrangler.openDuckDBTable`                | Open DuckDB Table                                              |
...
| -------------------------------------- | --------- | ---------------------------------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `openWrangler.defaultBackend`          | `string`  | `"auto"`                                       | `"auto"`, `"polars"`, `"duckdb"`, `"pandas"`, `"r"`       | Default file engine. Auto selects the first available Python engine in Polars, DuckDB, then Pandas order, subject to file-format and import-option support. If no compatible Python interpreter or engine is available, Auto tries R for supported files. Explicit Python engine choices, broken configured interpreter paths and unexpected environment errors do not select R. R opens local CSV, TSV, Parquet, JSONL/NDJSON and Excel files natively on Linux, macOS and Windows using Rscript, with format-specific limits and optional packages. It does not fall back to Python for unsupported files. Auto does not switch engines after a file-read error. Open DuckDB Table always uses DuckDB. |
| `openWrangler.defaultRLibrary`         | `string`  | `"base"`                                       | `"base"`, `"dplyr"`, `"data.table"`, `"collapse"`         | R library used for built-in cleaning and generated code when opening a new R dataframe. File restoration keeps its confirmed choice. Imports, profiles and exports use Open Wrangler's shared native R implementations. Optional packages must be installed in the exact R environment that owns the dataframe.                                                                                                                                                                                                                                                                                                                                                                                          |
| `openWrangler.fileStartMode`           | `string`  | `"editing"`                                    | `"editing"`, `"viewing"`                                  | Initial mode for file-backed sessions. Editing remains non-destructive until an explicit export. DuckDB database tables always open in viewing mode.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `openWrangler.enabledFileTypes`        | `array`   | `["csv","tsv","parquet","jsonl","xlsx","xls"]` | None                                                      | File types accepted by ordinary file launch commands and offered by their picker. The JSONL option includes both .jsonl and .ndjson files. Open DuckDB Table accepts any database filename independently of this setting. Explicit Reopen Editor With selection remains available.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `openWrangler.insightsOnOpen`          | `boolean` | `true`                                         | None                                                      | Show profiles in visible column headers when a grid opens. PySpark starts with this off; enable it from the grid when needed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `openWrangler.fetchBlockSize`          | `integer` | `200`                                          | None                                                      | Number of rows fetched for each virtual grid block.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `openWrangler.fetchColumnBlockSize`    | `integer` | `16`                                           | None                                                      | Number of dataframe columns fetched for each horizontal grid block.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `openWrangler.rscriptPath`             | `string`  | `""`                                           | None                                                      | Optional Rscript executable used for native R local files on Linux, macOS and Windows, and .R, .Rmd, and .qmd documents on macOS or Linux. When empty, Open Wrangler resolves Rscript from PATH.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `openWrangler.requestTimeoutMs`        | `number`  | `30000`                                        | None                                                      | Maximum time Open Wrangler waits for a runtime request other than session opening and native R data export. Notebook execution may continue after this wait expires.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
