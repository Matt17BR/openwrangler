...
- Operation-catalog consolidation is achieved: `protocol/openwrangler.v2.schema.json` is the sole hand-edited
  ...
  owners. The direct Native-R complete-catalog contract retains one independent, mutation-sensitive 30-operation
...
| 5. Core transforms                      | The generated [reference](reference.md) now includes fixed multi-output literal split and bounded portable regex-group extraction. It still lists no joins, pivot, explode/unnest, rank/window, or assertions; Formula remains binary arithmetic. | P2 in progress. Literal multi-output split and regex extraction are implemented; joins still wait for multi-source ownership. | Operation schema/catalog plus every editing-capable native engine.                                             | [P2](#p2-deterministic-reshape-and-validation)                                                                                                                  | Matching live and executable generated-code tests per engine, with lineage and type contracts.                                            |
