...
| 3. Cleaning history                     | Any committed step can be inspected, edited, or deleted. Editing uses its recorded input schema and one private candidate replay. Cleaning Undo removes the most recent committed step.                                                                                                                         | Implemented P1 history slice. Reordering committed steps has no product commitment. No hidden move primitive exists.                                      | Host plan-rewrite transaction plus Python and Native R selected-prefix preview owners.                         | [P1 history slice](#p1-fidelity-and-daily-use)                                                                                                                  | Prefix rewrite, suffix replay, stable IDs/lineage, rollback, persistence, generated-code, Activity Bar, and immutable-source tests.       |
