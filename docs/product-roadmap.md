...
- Operation-catalog consolidation is achieved: `protocol/openwrangler.v2.schema.json` is the sole hand-edited
  ...
  owners. The direct Native-R complete-catalog contract retains one independent, mutation-sensitive 29-operation
...
1. Multi-output literal split is implemented as one fixed 2–64-column operation. Regular-expression extraction,
   explode, and unnest remain planned.
...
| 5. Core transforms                      | The generated [reference](reference.md) now includes fixed multi-output literal split. It still lists no joins, pivot, explode/unnest, rank/window, regex extraction, or assertions; Formula remains binary arithmetic.     | P2 in progress. Multi-output split is implemented; joins still wait for multi-source ownership.                               | Operation schema/catalog plus every editing-capable native engine.                                             | [P2](#p2-deterministic-reshape-and-validation)                                                                                                                  | Matching live and executable generated-code tests per engine, with lineage and type contracts.                                            |
