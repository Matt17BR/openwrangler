...
- Operation-catalog consolidation is achieved: `protocol/openwrangler.v2.schema.json` is the sole hand-edited
  ...
  owners. The direct Native-R complete-catalog contract retains one independent, mutation-sensitive 31-operation
...
2. Pivot longer is implemented with exact scalar-type preservation and deterministic selected-column-major rows.
   Pivot wider and transpose remain planned where native engines can preserve types and lineage.
...
| 3. Cleaning history                     | Any committed step can be selected by stable ID, edited against its recorded input schema, or deleted through one private candidate replay; latest-step edit and Undo remain available.                                                                                              | Implemented P1 history slice. Reorder is explicitly deferred and no hidden move primitive exists.                                            | Host plan-rewrite transaction plus Python and Native R selected-prefix preview owners.                         | [P1 history slice](#p1-fidelity-and-daily-use)                                                                                                                  | Prefix rewrite, suffix replay, stable IDs/lineage, rollback, persistence, generated-code, Activity Bar, and immutable-source tests.       |
...
| 5. Core transforms                      | The generated [reference](reference.md) now includes fixed multi-output literal split, bounded portable regex-group extraction, and deterministic Pivot longer. It still lists no joins, Pivot wider, explode/unnest, rank/window, or assertions; Formula remains binary arithmetic. | P2 in progress. Literal multi-output split, regex extraction, and Pivot longer are implemented; joins still wait for multi-source ownership. | Operation schema/catalog plus every editing-capable native engine.                                             | [P2](#p2-deterministic-reshape-and-validation)                                                                                                                  | Matching live and executable generated-code tests per engine, with lineage and type contracts.                                            |
