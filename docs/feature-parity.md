...
The compatibility authority records VS Code on Linux, macOS, Windows at the **Fully qualified** tier through distinct exact 1.130.0 installed-performance and moving stable candidate lanes; the exact pin is not attributed to stable-channel jobs. Cursor 3.13.10 on Linux is a **Focused release seam**. Its separate Open Wrangler 1.2.0/Antigravity 1.107.0 Linux x64 record is **Smoke-tested** and does not promote other desktop forks above API-compatible. Native R 4.4.3 and 4.5.2 coverage is a **Focused release seam**, not full product qualification; its promotion remains tied to [issue #87](https://github.com/Matt17BR/openwrangler/issues/87).
...
| Dataset summary and quick insights                   |    Yes |    Yes | Done   | Engine-native exact Sum plus live Pandas/R UI; record:docs/testing.md                 |
...
Post-1.0 viewing-filter hardening keeps the completed filter surface usable as well as semantically correct. Focused React coverage proves that removing a final selected value removes the column filter itself, changing per-column logic cannot create an empty filter, and **Filter rows** stays disabled for an effective-empty query. A two-column interaction keeps every active filter visible above the grid and in the Filters drawer, removes one typed value or predicate without disturbing siblings, and preserves sorts on the same or another column. **Clear filters** retains every sort, while a separate **Undo latest filter** uses a 20-entry stack of correlated successful page confirmations; failed, cancelled, superseded, session-replaced, recovered, and cleaning-mutation responses cannot create stale undo entries. The native Filters tree exercises the same whole-column removal through the host/webview action boundary.
...
The released v1.1.1 notebook UX prepares automatic Pandas/Polars MIME formatters when a trusted, visible Jupyter notebook has a user-started kernel, rather than waiting for the first Open Wrangler command. The current preview applies the same preparation and live-variable action to Python Interactive windows. Stable Jupyter lookup never creates a kernel, API-opened background notebooks remain untouched, and a visible notebook change bypasses retry backoff so a newly available kernel is handled promptly. When the first Python result finishes before the formatter is ready, Open Wrangler carries the kernel observed at execution start through a type check of that exact `execute_result`. The cell action then opens the matching `Out[executionOrder]` value without rerunning the cell; display output, streams, Styler, and unrelated HTML have no dead action. If Microsoft Data Wrangler is installed, the default `ask` preference requires the user to choose which extension owns automatic dataframe previews before Open Wrangler registers a formatter; the provider remains changeable and a switch applies to new or restarted kernels. The notebook-toolbar action uses a bounded, kernel-backed QuickPick populated from canonical runtime types instead of asking users to type a variable name. Every discovery and launch retains the exact originating `NotebookDocument`, rejects duplicate same-URI document objects, and never retargets after focus changes or a kernel switch.

Inline MIME v2 output shows every captured column and pages the captured rows at 10, 20, 50, or 100 rows per page. A named dataframe links directly to that variable. An unassigned Pandas or Polars result, such as `frame.tail()`, receives an opaque kernel-local link while the result is alive. In both cases the single **Open in Open Wrangler** action opens the complete current value through the exact originating notebook and kernel; it never substitutes the saved capture. An older saved output without a live link stays readable inline and shows no button that cannot work. The notebook-toolbar and Jupyter Variables workflows remain additional live entry points. Focused unit, renderer, provenance, provider-conflict, restart, and packaged-editor acceptance defined in `docs/testing.md` must be green before this candidate is released.
...
from the Interactive toolbar in VS Code or the editor-title action in Cursor without rerunning a cell or creating
another window.
...
after a cell finishes. For an official vscode-R terminal, Operations reads vscode-R's current dataframe metadata
without running a command. Opening a listed frame or choosing Refresh creates the native connection; later updates use
...
the separately reviewed performance record, the installed all-32 matrix passes, and the following stable-authoring
...
detects the exact cursor-owned fence and runs only its chunk in the corresponding R or Jupyter session.
Python routing follows the document executor: R Markdown and knitr/reticulate Quarto stay in a pinned exact R
terminal, while Jupyter Quarto uses the exact resulting Interactive Window. Conflicting or unsupported executor
metadata fails closed, required integrations are preflighted before session acquisition, and both the source origin
...
Transform by example accepts ordered stable source references and aligned example input/output arrays. Native R uses
the shared deterministic candidate order to synthesize one canonical bounded program, stores that program in the
retained step, and uses it unchanged for evaluation, executable generated R, edit-latest, replay, inspection, and
undo. Strict scalar and reference validation rejects stale or incompatible inputs, unsafe whole numbers, and
structural signed zero. The shared 16-source, 64-example, 256-node, depth-64, 64-concat, 8-KiB-value, and 64-KiB-total
UTF-8 limits apply before synthesis and after canonicalization. Exact integral arithmetic uses the shared
`-(10^38 - 1)` through `10^38 - 1` envelope before R's exact publishable integer type is chosen. Direct contracts
exercise base `data.frame`, tibble, `data.table`, `collapse::qDF()`, `qTBL()`, and `qDT()` without changing the source.

Custom code accepts at most 64 KiB of exact UTF-8 R source and rejects NUL, blank/comment-only input, and parse errors
before evaluation. It executes trusted arbitrary R in the selected environment; it is not a sandbox. The isolated
input is available as `df`, and the code must create an exact local, non-active `result` with at least one column and
the same canonical base-data-frame, tibble, or `data.table` flavor. Rows, columns, row names, and a `data.table` key may
change. Duplicate result names reuse input lineage in FIFO order, new columns receive deterministic step identities,
and every result row receives a fresh identity. Preview reconciles filters and sorts against the result schema and
reports a full row-replacement diff. Live and generated R have the same result for deterministic code, input, and
environment. Source/result publication is atomic, but deliberate filesystem, network, global-environment, or
aliased-object side effects are outside that transaction.

Min-max scale accepts integer, double, and `integer64` columns and returns doubles from 0 to 1. Constant finite
columns become zero; missing and non-finite values become missing output. The result can replace the source column or
use a new column. Keyed `data.table` columns require the latter.
...
regex extraction is operation 30, Pivot longer is operation 31, and Pivot wider is operation 32. A dedicated direct R contract now executes the strict ordered 32-operation catalog and each
...
execution of all 32 operations and reviewed performance evidence remain outstanding. The focused value targeted
...
Pivot wider contract requires one exact text/factor key, one compatible scalar value, and 2–64 ordered typed keys
with explicit output names. It retains identifier columns and identities, rejects duplicate identifier/key pairs,
emits first-seen identifier groups with typed nulls for missing declared combinations, and never discovers columns or
aggregates duplicates implicitly. Its packaged Python and Native R acceptance recreates the renderer, hydrates the
saved operation, and exercises non-latest edit and delete with suffix replay, committed-boundary undo, and immutable
source restoration. The enclosing packaged flows then close every editor and require zero retained sessions and a
stopped runtime. The catalog remains **Partial** for its explicitly named installed and DuckDB gaps;
transpose, explode, and unnest remain planned.
...
picker still checks base, tibble, `data.table`, and supported `collapse` objects. Native R and cross-language tests
cover all supported operations, plus class and key behavior for tibbles and data tables. They also cover row identity,
compound sort priority, typed filtering, mixed plans, ordered selection, type conversion, Unicode character counts,
native R text casing and replacement, `NA` preservation, stable retained and derived identities, duplicate names,
...
A separate pair of focused Linux VS Code and Cursor phases is configured to require realistic `.Rmd` and `.qmd` fixtures with
first-line YAML, ignored prose, a relative CSV read, native editing, and a generated fenced R cell. Its Quarto
title-action journey pins the cursor in
...
explicit R-document command explains that no R code chunk was found. Packaged Linux runs cover all three document
...
the strict ordered 32-operation live/generated catalog and exact copy/save bytes for distinct executable
...
no-retry contracts remain unchanged; exhaustive installed execution of all 32 operations and a reviewed performance
...
ownership and cleanup. The fan-in failed, so run #78 created no `v1.99.6` tag, GitHub prerelease, Open VSX package, or
Azure Marketplace package. Preview [run #79](https://github.com/Matt17BR/openwrangler/actions/runs/31859989213) later
passed from exact protected `main` commit `4ed4d8d4422040dd5f1bcaae274a41fd3fd9cef8` and published `v1.99.6` to all
three destinations. That successful artifact still contains the historical 26-operation catalog and is not evidence
for either operation added in 1.99.7. Run #78 remains failed evidence rather than a source for reinterpreting run #79
or either new operation.
...
| Complete 32-operation catalog                | Yes                 | Partial | All kinds native/generated; packaged group matrix green | Full installed all-31 input-operation matrix, fill-missing journey, and DuckDB semantic edge matrix |
...
- Pull-request validation retains npm and Python vulnerability audits. The default PR checks always run
  `invariant-core`; its four conservative outputs select R 4.5, canonical package/editor, visual/accessibility, and
  Windows unique-risk owners. Scheduled/manual Cross retains platform checks and the R 4.4 qualification. Protected-branch
...
Header-profile filter visibility, 2026-08-08:

- Clicking a category or histogram bin in a grid header now opens **Column profiles** for that column and shows the
  active filter before the replacement profile finishes loading. The same surface exposes **Clear** immediately.
- Focused React coverage applies and clears the filter through the header chart and verifies the shared view model
...
Shared profile values, 2026-08-10:

- A single Counts/% setting now controls compact header summaries and the selected-column panel. It is available in
  the grid footer before the panel is opened, and changing it in either place updates both surfaces.
- The visible denominator sentence has been removed. Every value still exposes its exact count and percentage through
  its tooltip and accessible name, including sampled distributions.
- Truncated categorical and string profiles show **More values…**, which opens the existing bounded value browser
  without starting another exact profile.
