...
- Keep release numbers out of the README performance prose. The dated report link carries the exact product versions,
  timings, and method.
...
Daily previews instead link the previous preview, list merged PRs once each and keep long lists behind **Read more**,
using the frozen inputs described in [Releasing](releasing.md). They omit duplicate release headings and build/test
boilerplate. The checks prove which text will ship, not whether a person read it. Review curated prose in the release change.
