...
| `Checkout lifecycle contracts`      | agent checkout tooling       | focused disposable-repository and worktree lifecycle contracts                                                               | none                             |
...
The workflow-structure contract has exactly one pull-request owner, `Fast feedback`. The checkout-lifecycle contract
also has a focused conditional owner for its exact internal-tooling tier; the full product tier still runs it within
the portable corpus. All other general or Linux-owned `scripts/*.test.mjs` contracts run under `Contract tests`.
...
3. Mark one coherent slice ready. The `ready_for_review` activity reruns every pull-request workflow at the same SHA,
   ...
   non-packaged docs, source plus focused checkout-lifecycle contracts for that exact internal tool, source plus
   canonical packaging for the four shipped documents, or the complete product matrix for every other change.
...
package-only changes, checkout-lifecycle changes, and draft feedback now have bounded fail-closed paths. Extracting the shared
