...
Only `Fast feedback` executes substantive work on a documentation-only pull request, so formatting, ESLint, strict TypeScript, generated protocol/reference/docs freshness, license inventory, and workflow contracts still run. Canonical packaging, runtime/coverage, visual, native editor, Cursor, and Jupyter lanes use one job-level condition and retain their existing check names as explicit skipped results. This avoids hundreds of duplicated step predicates. The fail-closed aggregate accepts that skipped result set only alongside an exact successful documentation classification. On a missing or invalid classification, the condition defaults to running the full lane and the aggregate still fails the classifier.
...
Cross-platform runtime matrices and CodeQL remain separate workflows and separately protected contexts. Their exact job and matrix names are unchanged. Each workflow performs the same bounded classification; docs-only cells are emitted as skipped contexts, while a failed or non-boolean classification forces every normally required cell to start and fail its first gate. Substantive PRs and all non-PR events therefore retain the complete native runtime and CodeQL evidence. The CI aggregate does not claim to summarize a workflow it cannot depend on.
