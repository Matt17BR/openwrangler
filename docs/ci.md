...
next complete matrix. Stable metadata, tag publication, and Marketplace recovery derive their permitted branch from
the numeric version rather than trusting a caller-supplied ref. The live `release/1.x` branch was cut from
...
The v1.2.1 audit found another repeated run outside pull requests. A normal released change used about 82.5 minutes of
runner time before merge, another 70.2 after the protected push, and about 73.3 for the release candidate. The post-merge
matrix tested the same tree that had just passed the strict, up-to-date pull-request ruleset, but it did not guard a
merge or a release. Protected pushes now run only `Fast feedback`; ready pull requests and exact release candidates
still run their complete matrices.
