...
run preserves it. If the run history cannot be read, the check fails instead of assuming nothing changed.
...
list. Stable releases and historical manual previews keep their checked-in curated notes.
...
Preview packaging runs only on the first attempt of a workflow run. If packaging fails, fix `main` as needed and wait
for the next scheduled run; rerunning the package job is refused before checkout or setup. If the GitHub
**Publish preview** job fails, rerun only that job in the same workflow run while its stable-tag binding is still
current. It reconstructs the same source and reuses the same artifact and frozen notes inputs. Publication revalidates the baseline's tag and provenance and requires the
...
stable tag is now reachable, discard the old candidate and wait for the next scheduled run.
...
maintainers review whether the cited evidence supports them. The workflow also requires the default IRkernel notebook
journey in desktop VS Code on Linux, macOS and Windows. Each platform downloads the same run's canonical triple,
checks it against the exact source before and after acceptance, and builds only the test harness. The supplied VSIX
is never rebuilt. Existing private R preparation, package checks, phase deadlines, failure diagnostics and cleanup
...
candidate whose source remains in protected `main`. For a new release tag, the candidate source must still be the tip
of `main`: hold further merges from qualification until that tag is published. Exact existing-tag recovery remains
valid after `main` advances.
