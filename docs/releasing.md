...
retain distinct sealed failure diagnostics. Only the focused
...
notebook and remote-only selectors do not prepare or install them. Acquisition may retry only when the initial `fetch` promise rejects
...
pinning nor supply-chain evidence. The R contract, ordinary packaged-editor journey, both focused artifact
revalidations, the active-terminal journey, and the R Markdown/Quarto journey remain required release evidence.
