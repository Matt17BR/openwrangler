...
@pytest.mark.parametrize(
    ...
    [
        ...
        "signed-column-power",
        "signed-column-power-uint64",
        "signed-unsigned-column-power",
        "uint64-negative-column-add",
        "uint64-negative-column-subtract",
        "uint64-mixed-column-add",
        "uint64-mixed-column-subtract",
        "uint64-reversed-column-add",
        "uint64-reversed-column-overflow-add",
        "decimal-multiply",
        "decimal-fractional-multiply",
        "decimal-divide",
...
def test_arrow_formula_capacity_publishes_replays_exports_and_preserves_failed_state(
    ...
    fractional_product = family == "decimal-fractional-multiply"
    ...
    uint64_result_power = family == "signed-column-power-uint64"
    column_power = family == "signed-column-power" or unsigned_column_power or uint64_result_power
    signed_power = family in {"signed-even-power", "signed-odd-power", "signed-wide-odd-power"} or column_power
    ...
    if signed_subtract:
        ...
        if family == "uint64-reversed-column-overflow-add":
            unsigned_values = [0, 2**64 - 1, None]
    series = (
    ...
    if fractional_product:
        series = pd.Series(
            [Decimal("0.5"), Decimal("-0." + "9" * 38), None], dtype=pd.ArrowDtype(pa.decimal128(38, 38))
        )
    if signed_power:
        series = pd.Series([0, -1 if wide_odd_power else 2 if odd_power else -3, None], dtype="int64[pyarrow]")
        if column_power:
            series = pd.Series([1, 2, None] if uint64_result_power else [-6, 5, None], dtype="int8[pyarrow]")
        if odd_power or column_power:
    ...
    if column_power:
        exponent = 63 if uint64_result_power else 7 if unsigned_column_power else 3
        exponent_dtype = "UInt8" if unsigned_column_power else "int8[pyarrow]"
        frame["divisor"] = pd.Series([exponent, exponent, None], dtype=exponent_dtype)
        frame["unsafeAdjustment"] = pd.Series([exponent, 64 if uint64_result_power else 28, None], dtype=exponent_dtype)
    if odd_power:
    ...
    try:
        ...
        if fractional_product:
            ...
            operation["params"].update(operator="multiply", rightColumn=operation["params"]["leftColumn"])
        if decimal_unit:
        ...
        if signed_power:
            ...
            operation["params"]["rightColumn"] = {"id": columns[1]["id"], "name": columns[1]["name"]}
            if signed_left:
                operation["params"]["leftColumn"], operation["params"]["rightColumn"] = (
                    operation["params"]["rightColumn"],
        ...
        if signed_add:
            ...
            unsigned_expected = (
                [1, 2**63, None]
                if uint64_result_power
                else [-279936, 78125, None]
                if unsigned_column_power
                else [-216, 125, None]
            )
        elif signed_power:
        ...
        expected = pd.Series(
            ...
            dtype=(
                "int64[pyarrow]"
                if column_power
                and not uint64_result_power
                or wide_odd_power
                or signed_subtract
                or family == "uint64-negative-multiply"
                else "uint64[pyarrow]"
            )
            if unsigned or signed_power or signed_add
        ...
        if fractional_product:
            with localcontext() as context:
                context.prec = 100
                squared = [Decimal("0.25"), Decimal("0." + "9" * 38) ** 2, None]
            expected = pd.Series(squared, index=frame.index, name="result", dtype=pd.ArrowDtype(pa.decimal256(76, 76)))
        ...
        if signed_power:
            ...
            invalid["params"]["operator"] = operation["params"]["operator"]
            invalid["params"]["leftColumn" if signed_left else "rightColumn"] = {
                "id": columns[2]["id"],
                "name": columns[2]["name"],
            }
            if signed_left:
        ...
        if (
            ...
            pd.testing.assert_series_equal(session.committed["result"], expected)
        if signed_left or signed_power:
            redone = manager.redo_step(session_id, session.revision, 0, 1)
            assert redone["action"] == "redo"
            assert session.plan == [operation, history]
            pd.testing.assert_series_equal(session.committed["result"], expected)
    finally:
        manager.close_all()
