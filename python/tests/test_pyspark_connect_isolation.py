...
def test_connect_request_failure_requires_connect_and_safe_structured_accessors() -> None:
    engine = PySparkEngine()
    engine._indexed_frame = object()
    lost = FakeSparkConnectError(condition="INVALID_HANDLE.SESSION_NOT_FOUND")
    assert engine.classify_request_failure(lost) is None

    engine._indexed_frame = FakeConnectFrame()
    assert engine.classify_request_failure(FakeSparkConnectError(accessors_fail=True)) is None
