    ...
    if series.dtype == object and counts.index.dtype.kind in {"M", "m"}:
        ...
        for value in series.array:
            ...
            if isinstance(value, (np.datetime64, np.timedelta64)):
                ...
                non_fixed_duration = dtype.kind == "m" and unit in {"Y", "M", "generic"}
                if non_fixed_duration or multiplier != 1 or unit in {"ps", "fs", "as"}:
                    if dtype.kind == "m" and unit == "ns":
                        continue
                    if np.isnat(value) or (not non_fixed_duration and int(value.view(np.int64)) == 0):
                        continue
                    raise EngineError(
...
class PandasEngine(DataFrameEngine):
    ...
    def header_stats(self, frame: Any) -> dict[str, Any]:
        ...
        df = logical
        missing_by_column = []
        missing_row_mask = np.zeros(len(df), dtype=bool) if native_frame else None
        for position, column in enumerate(df.columns):
            ...
            if missing_row_mask is None:
                ...
                np.logical_or(missing_row_mask, missing, out=missing_row_mask)
                del missing
            missing_by_column.append({"column": str(column), "count": count})
        ...
        missing_rows = (
            int(df.isna().any(axis=1).sum()) if missing_row_mask is None else int(np.count_nonzero(missing_row_mask))
        )
    ...
    def _apply_bound_filter_model(self, frame: Any, model: Any) -> Any:
        ...
        for column_filter in model.get("filters", []):
            ...
            column_type = _pandas_semantic_type(series)
            if column_filter.get("type") != column_type:
                raise EngineError(
        ...
        if needs_nullable_result_helpers or "grouped" in fill_strategies or needs_rank_helpers:
            lines.extend(
                [
                    ...
                    "    if isinstance(series.dtype, pd.ArrowDtype):",
                    "        zeros = series.eq(0).fillna(False)",
                    "        if zeros.any():",
                    "            series = series.mask(zeros, series.iloc[zeros.argmax()])",
                    "    return series",
                    "",
                    "",
                ]
        ...
        if needs_nullable_result_helpers:
            lines.extend(
                [
                    ...
                    (
                        "    if not mask.any() and not "
                        "(pd.api.types.is_float_dtype(series.dtype) and series.isna().any()):"
                    ),
    ...
    def _compile_step(self, step: Mapping[str, Any], index: int, *, output_name: str | None = None) -> list[str]:
        ...
        if kind == "groupBy":
            ...
            lines = [
                ...
                f"{prefix}        _nan_mask = (np.isnan(_series) & _series.notna()).fillna(False)",
                f"{prefix}        if _nan_mask.any():",
                f"{prefix}            {selected}[_position] = _series.mask(_nan_mask, pd.NA)",
            ...
            for aggregation_index, (_position, operation, _alias) in enumerate(aggregations):
                if operation == "sum":
                    ...
                    lines.extend(
                        [
                            f"{prefix}if pd.api.types.is_float_dtype("
                            f"{source}[{value_names[aggregation_index]!r}].dtype):",
                            f"{prefix}    df.isetitem({output_position}, "
...
def _pandas_group_by_positions(
    ...
    for position, series in selected.items():
        if pd.api.types.is_float_dtype(series.dtype):
            nan_mask = (np.isnan(series) & series.notna()).fillna(False)
            if nan_mask.any():
                selected[position] = series.mask(nan_mask, pd.NA)
    ...
    for aggregation_index, (_position, operation, _alias) in enumerate(aggregations):
        if operation == "sum" and pd.api.types.is_float_dtype(source[value_names[aggregation_index]].dtype):
            output_position = len(key_positions) + aggregation_index
            result.isetitem(
...
class _PandasRowAxisFormatter:
    ...
    def _write_top_level_scalar(self, value: Any) -> None:
        ...
        if value_type is str or value_type is trusted_string_type:
            self._add(cast(str, value))
            return
        if value_type is bytes or value_type is trusted_bytes_type:
            self._write_bytes(cast(bytes, value), quoted=False)
            return
        if value_type is int:
            self._add(self._bounded_integer_text(value))
            return
        if value_type is Decimal:
            if Decimal.is_nan(value):
                self._add("null")
            else:
                self._add(self._bounded_decimal_text(value))
            return
        cell = self._supported_scalar_cell(value, trusted_cell_types)
    ...
    def _write_json_value(self, value: Any, depth: int) -> None:
        ...
        if value_type is dict:
            ...
            return
        if value_type is str or value_type is trusted_string_type:
            self._write_json_string(cast(str, value))
            return
        if value_type is bytes or value_type is trusted_bytes_type:
            self._write_bytes(cast(bytes, value), quoted=True)
            return
        if value_type is int:
            self._add(self._bounded_integer_text(value))
            return
        if value_type is Decimal:
            if Decimal.is_nan(value):
                ...
                self._write_json_string(self._bounded_decimal_text(value, reserved_characters=2))
            return
        cell = self._supported_scalar_cell(value, trusted_cell_types)
        kind = cell["kind"]
        if kind == "null":
    ...
    def _write_json_mapping(self, value: dict[Any, Any], depth: int) -> None:
        ...
        try:
            ...
            for index, (key, item) in enumerate(normalized_items.items()):
                if index:
                    self._add(",")
                self._write_json_string(key)
                self._add(":")
                self._write_json_value(item, depth + 1)
            self._add("}")
        finally:
            self._active_compounds.remove(identity)
    ...
    def _mapping_key(self, value: Any) -> str:
        value_type = type(value)
        trusted_cell_types, trusted_string_type, _trusted_bytes_type = _pandas_row_axis_trusted_scalar_types()
        if value_type is str or value_type is trusted_string_type:
    ...
    def _bounded_decimal_text(self, value: Decimal, *, reserved_characters: int = 0) -> str:
        if Decimal.__sizeof__(value) > _MAX_ROW_AXIS_DECIMAL_STORAGE_BYTES:
            self._fail_text_bound()
        decimal_tuple = Decimal.as_tuple(value)
        if len(decimal_tuple.digits) > _MAX_ROW_AXIS_DECIMAL_DIGITS:
            self._fail_text_bound()
        length = self._decimal_text_length(decimal_tuple.sign, len(decimal_tuple.digits), decimal_tuple.exponent)
        if length > self._remaining_characters - reserved_characters:
            self._fail_text_bound()
        text = _pandas_row_axis_decimal_text(value)
        if len(text) != length:
            raise EngineError(f"{self._purpose} has an inconsistent decimal display value.")
        return text

    @staticmethod
    def _decimal_text_length(sign: int, digit_count: int, exponent: int | str) -> int:
        if exponent == "F":
            return sign + len("Infinity")
        if exponent in {"n", "N"}:
            return sign + len("NaN" if exponent == "n" else "sNaN") + digit_count
        if not isinstance(exponent, int):
            raise EngineError("Pandas row-axis decimal metadata is malformed.")
        left_digits = exponent + digit_count
        dot_place = left_digits if exponent <= 0 and left_digits > -6 else 1
        if dot_place <= 0:
            mantissa_length = 2 - dot_place + digit_count
        elif dot_place >= digit_count:
            mantissa_length = dot_place
        else:
            mantissa_length = digit_count + 1
        if left_digits == dot_place:
            exponent_length = 0
        else:
            exponent_value = abs(left_digits - dot_place)
            exponent_digits = 1
            while exponent_value >= 10:
                exponent_value //= 10
                exponent_digits += 1
            exponent_length = 2 + exponent_digits
        return sign + mantissa_length + exponent_length
...
def _pandas_group_nulls(series: Any, null_mask: Any) -> Any:
    ...
    mask = np.asarray(null_mask, dtype=bool)
    if not mask.any() and not (pd.api.types.is_float_dtype(series.dtype) and series.isna().any()):
...
def _pandas_group_key(series: Any) -> Any:
    ...
    if not nan_mask.any():
        return _pandas_nullable_string_copy(series)
    return _pandas_group_nulls(series, series.isna() | nan_mask)


def _pandas_prepare_float_group_key(series: Any) -> Any:
    import numpy as np
    import pandas as pd
    ...
    if nan_mask.any():
        series = series.mask(nan_mask, pd.NA)
    if isinstance(series.dtype, pd.ArrowDtype):
        # Arrow hashes zero signs separately; native numeric group equality does not.
        zeros = series.eq(0).fillna(False)
        if zeros.any():
            series = series.mask(zeros, series.iloc[zeros.argmax()])
    return series
