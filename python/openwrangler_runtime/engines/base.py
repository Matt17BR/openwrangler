...
class LivePagePayloadError(EngineError):
...
def _validate_live_page_text(value: Any) -> None:
    if not isinstance(value, str):
        return
    if len(value) > LIVE_PAGE_TEXT_CHARACTER_LIMIT:
        raise LivePagePayloadError(
            f"Live page text values may contain at most {LIVE_PAGE_TEXT_CHARACTER_LIMIT:,} Unicode code points. "
            "Request fewer rows or columns, or shorten large values."
        )
    try:
        value.encode("utf-8")
    except UnicodeEncodeError as error:
        raise LivePagePayloadError("Live page text values must be valid strict UTF-8.") from error
