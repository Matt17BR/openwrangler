...
- Cleaning sorts and filters, missing/duplicate-row rules, structural/formula/length steps, and categorical, text, numeric, and datetime value transforms target stable column identities instead of relying on displayed labels. Pandas executes those paths positionally—including in-place changes and categorical encoding—so two equal labels and non-string labels are not silently conflated; Polars and DuckDB receive verified native names only after binding. Viewing filters/sorts remain separate non-destructive state, and copying them into a cleaning step fails clearly if a displayed name is ambiguous. Editing a saved filter keeps its exact stable query by default and exposes replacement from the current view as an explicit choice. Edited output identities replay deterministically through later steps and undo. Private row IDs stay unreachable under case changes and Pandas MultiIndex labels, generated categorical/value outputs are collision-checked, DuckDB rejects case-only ambiguity, and every transformation must retain one visible column so runtime results, generated code, and exports preserve the same row-count semantics. The operation editor explicitly distinguishes a default multi-label prefix from an intentional empty prefix and round-trips empty find patterns. Group/aggregate and by-example duplicate/non-string addressing plus their complete installed-editor cases remain 1.0 acceptance gates.
- Validated engine-native edge semantics for per-column null sorting, missing/duplicate modes, Unicode casing and whitespace stripping, empty-pattern replacement, finite numeric scaling, nullable groups, and collision-safe categorical output. One-hot encoding accepts numeric, boolean, date, and text inputs; excludes blank, null, and NaN values consistently; and publishes generated columns in one deterministic order across engines.
...
The checked-in Data Wrangler 1.24.2 clean-room behavior matrix is the release gate. Several rows still require group/by-example duplicate/non-string operation coverage, installed-editor, Restricted Mode, released-Jupyter, performance, or cross-platform evidence named there, so this build remains an explicitly incomplete preview. Intentionally deferred scope is listed separately in `docs/feature-parity.md`.
