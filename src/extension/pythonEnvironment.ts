...
export async function probeDependencies(
  ...
  const program = [
    "import importlib,importlib.metadata,json,os,stat",
    "try:",
    ...
    " value=lambda item:(item.st_dev,item.st_ino,item.st_size,item.st_mtime_ns,item.st_ctime_ns)",
    " result=value(before)",
    " return result if stat.S_ISREG(opened.st_mode) and opened.st_nlink==1 and stat.S_ISREG(current.st_mode) and current.st_nlink==1 and value(opened)==result and value(current)==result else None",
