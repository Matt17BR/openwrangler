...
export async function probeDependencies(
  ...
  const program = [
    ...
    "   opened=os.fstat(child); expected=entry(named)",
    "   if not stat.S_ISDIR(opened.st_mode) or entry(opened)!=expected: return None",
    ...
    "  if not stat.S_ISREG(named.st_mode) or named.st_nlink!=1: return None",
    ...
    "  if not stat.S_ISREG(opened.st_mode) or opened.st_nlink!=1 or entry(opened)!=expected or entry(current_file)!=expected: return None",
    ...
    "   if not stat.S_ISDIR(named_ancestor.st_mode) or not stat.S_ISDIR(opened_ancestor.st_mode) or entry(named_ancestor)!=expected or entry(opened_ancestor)!=expected: return None",
    ...
    "   if bool(value.attributes&0x400) or is_directory==is_file or (is_file and value.links!=1): close(ctypes.c_void_p(handle)); return None",
    "   handles.append((handle,value))",
    "  for handle,expected in handles:",
    "   current_info=WinInfo()",
    "   if not info(ctypes.c_void_p(handle),ctypes.byref(current_info)) or current_info.attributes!=expected.attributes or current_info.links!=expected.links or win_value(current_info)!=win_value(expected): return None",
    "  return win_value(handles[-1][1])",
    " except BaseException: return None",
    " finally:",
    "  for handle,_value in reversed(handles): close(ctypes.c_void_p(handle))",
    "def identity(path):",
    ...
    "  supported=bool(valid and SpecifierSet and Version and owned(dist,module) and SpecifierSet(spec).contains(Version(version),prereleases=False))",
