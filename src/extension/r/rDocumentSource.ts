...
export function prepareRDocumentSource(filePath: string, source: string): PreparedRDocumentSource {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!fence) {
      ...
      if (/^(?:---|\.\.\.)[\t ]*$/u.test(line)) {
        throw new SyntaxError(
          `Open Wrangler supports one YAML metadata block starting on the first line (line ${index + 1}).`
        );
      }
      continue;
    }
...
function parseRChunkHeaderOptions(info: string): readonly Readonly<{ key: string; value: string }>[] {
  ...
  if (rest.startsWith(",")) {
    ...
    if (!first.includes("=")) {
      if (!/^[A-Za-z][A-Za-z0-9_.-]*$/u.test(first)) {
...
function rejectUnsupportedMarkdownContainer(line: string, lineNumber: number): void {
  ...
  if (trimmed.includes("\\[") || /(?:\$\$|\\[A-Za-z]+\s*[[{])/u.test(trimmed)) {
    throw new SyntaxError(
      `Open Wrangler cannot safely distinguish R cells inside display math or raw TeX near line ${lineNumber}.`
    );
  }
}
