...
export function writeReleasedRNotebook(
  ...
  const bindingProbe = [
    ...
    "    export_lifecycle <- get('export_lifecycle', envir = environment(runtime$agent$dispose), inherits = FALSE)",
    "    export_environment <- environment(export_lifecycle$dispose)",
    "    artifacts <- get('artifacts', envir = export_environment, inherits = FALSE)",
    "    export_root <- get('export_root', envir = export_environment, inherits = FALSE)",
    "    length(ls(envir = artifacts, all.names = TRUE)) + length(list.files(export_root, all.files = TRUE, no.. = TRUE))",
    "  }) else 0L,",
