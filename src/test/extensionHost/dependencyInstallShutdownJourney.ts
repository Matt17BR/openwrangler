...
export function createDependencyInstallShutdownJourney({
  ...
  return async function exerciseDependencyInstallShutdownLifecycle(testing: TestApi, python: string): Promise<void> {
    ...
    await withDependencyInstallLifecyclePython(
      python,
      async ({ directory, lifecycle, preserveDirectory }) => {
        ...
        try {
          ...
          assert.equal(
            await installButton.isVisible(),
            true,
            "The dependency lifecycle Install action must be visible."
          );
          assert.equal(
            await installButton.isEnabled(),
            true,
            "The dependency lifecycle Install action must be enabled."
          );
          ...
          assert.equal(
            ...
            true,
            "The private pip directory must remain owned until close."
          );
