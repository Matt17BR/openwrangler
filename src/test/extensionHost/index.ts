...
async function exerciseReleasedDataWranglerCoexistence(
  ...
  try {
    ...
    if (expectation.selection) {
      ...
      firstOutputReceipt = await captureDataWranglerFirstOutput(workbench, notebook, notebookEditor, ownershipSentinel);
      assert.equal(
        configuration.get("notebookPreviewProvider", "ask"),
        "ask",
...
async function waitForConnectedInlineDataframeOwner(
  ...
  do {
    ...
    if (matches.length === 1) {
      assertNotebookRendererLifecycle(workbench, browser);
      assertExactVisibleReleasedNotebookEditor(notebook, notebookEditor, "after resolving its exact dataframe output");
      return matches[0]!;
    }
