...
for (attribute_order_flavor in seq_along(by_example_attribute_order_sources)) {
  ...
  for (attribute_order_index in seq_along(by_example_attribute_order_specs)) {
    ...
    # Public By Example captures each helper result before the next step.
    # The native data.table capture drops element names on derived columns too.
    attribute_order_expected <- get(
      "snapshot", envir = real_capture_frame(attribute_order_expected), inherits = FALSE
    )
  }
...
for (latin1_flavor_index in seq_along(latin1_generated_sources)) {
  ...
  latin1_expected <- real_by_example_column_at(
    latin1_generated_source, 1L, "text", "text copy", "character", function(columns) columns[[1L]]
  )
  latin1_expected <- get("snapshot", envir = real_capture_frame(latin1_expected), inherits = FALSE)
  latin1_expected_names <- attr(latin1_expected$`text copy`, "names", exact = TRUE)
  ...
  assert_identical(
    ...
    latin1_expected_names,
    sprintf("generated by-example flavor %d diverged from captured output names", latin1_flavor_index)
  )
  if (inherits(latin1_generated_source, "data.table")) {
    assert_identical(latin1_expected_names, NULL, "the live data.table capture retained derived element names")
  } else {
    assert_identical(
      latin1_expected_names, c(NA_character_, enc2utf8(latin1_boundary)),
      "the live base-frame capture did not normalize output names"
    )
    assert_identical(
  ...
  assert_identical(
    ...
    attr(latin1_expected$text, "names", exact = TRUE),
    sprintf("generated by-example flavor %d diverged from captured retained names", latin1_flavor_index)
  )
  assert_identical(
...
generated_name_overflow_sources <- list(
  generated_name_overflow_source,
  data.table::as.data.table(generated_name_overflow_source),
  collapse::qDT(generated_name_overflow_source)
)
for (name_overflow_source in generated_name_overflow_sources) {
  data.table::setattr(base::.subset2(name_overflow_source, 1L), "names", oversized_element_name)
  generated_name_overflow_before <- serialize(name_overflow_source, NULL, version = 3L)
  source_environment$by_example_name_overflow <- name_overflow_source
  opened <- dispatch("openSession", list(
    sessionId = name_overflow_session, variableName = "by_example_name_overflow", page = page_window()
  ))
  assert_identical(opened$kind, "page", "the named source flavor did not open")
  preview <- dispatch("previewStep", list(
    sessionId = name_overflow_session, revision = 0L, step = latin1_step, page = page_window()
  ))
  assert_identical(preview$kind, "error", "live by-example accepted an oversized derived name")
  assert_identical(preview$code, "unsupported_frame", "the oversized output-name diagnostic code changed")
  assert_identical(
    preview$message, "byExample output name row 1 exceeds 8192 UTF-8 bytes",
    "live by-example failed before validating the oversized derived name"
  )
  invisible(dispatch("closeSession", list(sessionId = name_overflow_session)))

  generated_name_overflow_environment <- new.env(parent = baseenv())
  assign("by_example_latin1", name_overflow_source, envir = generated_name_overflow_environment)
  ...
  assert_identical(
    serialize(get("by_example_latin1", envir = generated_name_overflow_environment), NULL, version = 3L),
    generated_name_overflow_before,
    "failed generated output-name validation mutated its source"
  )
  assert_identical(
    serialize(source_environment$by_example_name_overflow, NULL, version = 3L), generated_name_overflow_before,
    "failed live output-name validation mutated its source"
  )
}
