...
import { inspectReleaseMetadata, NUMERIC_RELEASE_VERSION } from "./release-metadata.mjs";
...
const AUTOMATIC_MAIN_REASONS = new Set(["BatchedCI", "IndividualCI"]);
...
export function inspectMarketplaceRecoveryChange({ changedPaths, parentCommits }) {
  ...
  if (
    ...
    problems.push("Protected-main recovery requires an unambiguous bounded parent-commit list.");
  }
  ...
  if (
    ...
    problems.push("Protected-main recovery requires one unambiguous bounded changed-path list.");
  }
...
function recoveryChangeAtCommit(root, commit) {
  if (!FULL_COMMIT.test(commit)) {
    throw new Error("Protected-main recovery change detection requires one full Git commit.");
  }
  ...
  if (
    ...
    throw new Error("Protected-main recovery could not bind one exact commit ancestry.");
  }
  if (parentCommits.length !== 1) {
  ...
  if (output.length > 0 && output.at(-1) !== 0) {
    throw new Error("Protected-main recovery received an unterminated Git path list.");
  }
  ...
  const changedPaths = encodedPaths.map((encoded) => {
    ...
    if (path.length === 0 || !Buffer.from(path, "utf8").equals(bytes)) {
      throw new Error("Protected-main recovery received a non-canonical Git path.");
    }
    return path;
  });
  return Object.freeze({
...
export function inspectMarketplaceRecoverySource(packageJson) {
  ...
  try {
    ...
    problems.push("Protected-main recovery requires one bounded strict package.json.");
  }
  if (
    ...
    problems.push("Protected-main recovery must describe Matt17BR.openwrangler.");
  }
  const version =
    typeof manifest === "object" &&
    manifest !== null &&
  ...
  if (version === undefined) {
    problems.push("Protected-main recovery requires one canonical numeric package version.");
  }
...
export function inspectMarketplaceReleaseIntake({
  ...
  const automaticMain =
    existingReleaseTag === "" && sourceBranch === MAIN_REF && AUTOMATIC_MAIN_REASONS.has(buildReason);
  ...
  if (historical) {
    ...
    releaseMayPrecedeAutomation = true;
    if (resolvedMainCommit !== sourceCommit) {
      problems.push("Automatic Marketplace recovery must run from the current public protected-main commit.");
    }
    ...
    problems.push(
      "Automatic Marketplace promotion accepts only a canonical numeric Git tag ref or reviewed protected-main recovery."
    );
  } else {
  ...
  if (result.promote === false) {
    if (result.noOpReason === "missing-tag" && result.releaseTag !== undefined) {
      return Object.freeze([
        ...
        `No immutable release tag ${result.releaseTag} exists for the current package version; protected-main recovery completed without Marketplace promotion.`
      ]);
    }
...
function runCli() {
  ...
  const automaticMain =
    existingReleaseTag === "" && sourceBranch === MAIN_REF && AUTOMATIC_MAIN_REASONS.has(buildReason);
  const recoveryChange = automaticMain
    ? recoveryChangeAtCommit(root, checkedOutCommit)
    : { changedPaths: undefined, parentCommits: undefined };
  const inspectedRecoveryChange = automaticMain
    ? inspectMarketplaceRecoveryChange(recoveryChange)
    : { problems: [], relevant: false };
  const relevantAutomaticMain =
    automaticMain && inspectedRecoveryChange.problems.length === 0 && inspectedRecoveryChange.relevant;
  const recovery = relevantAutomaticMain
  ...
  const releaseCommit =
    typeof releaseTag === "string" && RELEASE_TAG.test(releaseTag)
      ? resolveTagCommit(root, releaseTag, { optional: relevantAutomaticMain })
      : undefined;
  const result = inspectMarketplaceReleaseIntake({
    buildReason,
    checkedOutCommit,
    currentMainCommit: existingReleaseTag !== "" || relevantAutomaticMain ? currentMainCommit() : undefined,
    currentPackageJson,
    existingReleaseTag,
    recoveryChange,
    releasePackageJson: releaseCommit === undefined ? currentPackageJson : packageJsonAtCommit(root, releaseCommit),
    remoteTagCommit:
      (existingReleaseTag !== "" || relevantAutomaticMain) && releaseTag !== undefined
