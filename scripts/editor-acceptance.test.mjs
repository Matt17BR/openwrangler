...
posixTest("comparison-study phases pass only verified bridge and source paths", async () => {
  const directory = await mkdtemp(join(tmpdir(), "openwrangler-comparison-study-environment-"));
  const privateRoot = join(directory, "private");
  const environment = { PATH: "/safe/bin" };
  const resultPath = join(privateRoot, "result.json");
  const bridgeDirectory = join(privateRoot, "bridge");
  const requestPath = join(bridgeDirectory, "request.json");
  const acknowledgementPath = join(bridgeDirectory, "ack.json");
  const sourcePath = join(privateRoot, "fixture.csv");
  let launchedEnvironment;
  try {
    configureEditorAcceptanceTempRoot(privateRoot, environment);
    await mkdir(bridgeDirectory, { mode: 0o700 });
    await writeFile(sourcePath, "c00\n0\n", { mode: 0o600 });
    await runEditorAcceptancePhase(
      {
        editor: { name: "VS Code", key: "vscode", version: "1.130.0", executable: "fake-editor" },
        workspace: privateRoot,
        userData: join(privateRoot, "u"),
    ...
    assert.deepEqual(
      Object.fromEntries(Object.entries(launchedEnvironment).filter(([key]) => key.startsWith("OPEN_WRANGLER_STUDY_"))),
      {
        ...
        OPEN_WRANGLER_STUDY_PUBLIC_SURFACE_AVAILABILITY: "available"
      }
    );
    assert.throws(
...
posixTest("comparison-study phases reject stale bridge entries and sources outside their private root", async (t) => {
  await t.test("stale request", async () => {
    const directory = await mkdtemp(join(tmpdir(), "openwrangler-comparison-study-stale-"));
    const privateRoot = join(directory, "private");
    const environment = { PATH: "/safe/bin" };
    const bridgeDirectory = join(privateRoot, "bridge");
    const requestPath = join(bridgeDirectory, "request.json");
        ...
        runEditorAcceptancePhase(
          {
            ...
            python: join(privateRoot, "python"),
            phase: "comparison-study-open-wrangler-trial",
            resultPath: join(privateRoot, "result.json"),
            comparisonStudyEnvironment: {
              requestPath,
              acknowledgementPath,
              sourcePath,
          ...
          { platform: "linux", environment, spawnProcess: () => assert.fail("stale bridge launched") }
        ),
        /request path must be absent before launch/u
      );
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  ...
  await t.test("outside source", async () => {
    const directory = await mkdtemp(join(tmpdir(), "openwrangler-comparison-study-outside-"));
    const privateRoot = join(directory, "private");
    const environment = { PATH: "/safe/bin" };
    const bridgeDirectory = join(privateRoot, "bridge");
    const requestPath = join(bridgeDirectory, "request.json");
    const acknowledgementPath = join(bridgeDirectory, "ack.json");
    const sourcePath = join(directory, "outside.csv");
    try {
      configureEditorAcceptanceTempRoot(privateRoot, environment);
      await mkdir(bridgeDirectory, { mode: 0o700 });
      await writeFile(sourcePath, "c00\n0\n", { mode: 0o600 });
      await assert.rejects(
        runEditorAcceptancePhase(
          ...
          { platform: "linux", environment, spawnProcess: () => assert.fail("outside source launched") }
        ),
        /source must stay inside its private runner root/u
      );
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
