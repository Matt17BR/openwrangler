...
test("editor phases validate and forward exact R, candidate, and PySpark denial selectors", async () => {
  ...
  try {
    await assert.rejects(
      ...
      /test selector must be unset, "candidate-compatibility-seam", "pyspark-prerelease-denial", "core-operations", "categorical-operations", "value-operations", "pivot-wider", "kernel-restart", "native-frames", "interactive-terminal", or "literate-documents"/u
    );
    await assert.rejects(
