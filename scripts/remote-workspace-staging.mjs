...
function directoryReceipt(path) {
  ...
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
    throw new Error("The generated CommonJS closure requires one regular directory root.");
  }
  const canonical = realpathSync(path);
  if (canonical !== path) {
    throw new Error("The generated CommonJS closure requires canonical directory paths.");
  }
  return Object.freeze({
    dev: metadata.dev,
    ino: metadata.ino,
...
function canonicalDirectoryPath(path, label) {
  ...
  if (canonical !== bounded || !metadata.isDirectory() || metadata.isSymbolicLink()) {
    throw new Error(`The generated CommonJS closure requires one canonical regular ${label}.`);
  }
  return canonical;
}
...
function boundedAbsolutePath(path, label) {
  if (typeof path !== "string" || !isAbsolute(path) || path.length <= 0 || path.length > 16_384) {
    throw new Error(`The generated CommonJS closure requires one bounded absolute ${label} path.`);
  }
  return resolve(path);
}
