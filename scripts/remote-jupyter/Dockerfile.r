...
ARG COLLAPSE_REPOSITORY=https://p3m.dev/cran/__linux__/noble/2026-06-01
...
ENV PATH=/opt/openwrangler/venv/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
    ...
    COLLAPSE_REPOSITORY=${COLLAPSE_REPOSITORY} \
...
RUN python -I -m pip install \
    ...
    && Rscript --vanilla -e 'repository <- Sys.getenv("R_REPOSITORY"); collapse_repository <- Sys.getenv("COLLAPSE_REPOSITORY"); packages <- c("IRkernel", "jsonlite", "rlang", "tibble", "data.table"); install.packages(packages, repos = repository, Ncpus = 2L); install.packages("collapse", repos = collapse_repository, Ncpus = 2L); expected <- c(IRkernel = Sys.getenv("IRKERNEL_VERSION"), jsonlite = Sys.getenv("JSONLITE_VERSION"), rlang = Sys.getenv("RLANG_VERSION"), tibble = Sys.getenv("TIBBLE_VERSION"), data.table = Sys.getenv("DATA_TABLE_VERSION"), collapse = Sys.getenv("COLLAPSE_VERSION")); actual <- vapply(names(expected), function(package) as.character(utils::packageVersion(package)), character(1)); stopifnot(as.character(getRversion()) == "4.5.2", identical(actual, expected)); IRkernel::installspec(user = FALSE, prefix = "/opt/openwrangler/venv", name = "openwrangler-r-remote-acceptance", displayname = "R (Open Wrangler Remote)")'
