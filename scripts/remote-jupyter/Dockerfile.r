...
RUN Rscript --vanilla -e 'options(HTTPUserAgent = sprintf("R/%s R (%s)", getRversion(), paste(getRversion(), R.version["platform"], R.version["arch"], R.version["os"]))); repository <- Sys.getenv("R_REPOSITORY"); supplemental_repository <- Sys.getenv("R_SUPPLEMENTAL_REPOSITORY"); packages <- c("IRkernel", "jsonlite", "rlang", "tibble", "data.table", "arrow", "clock"); install.packages(packages, repos = repository, Ncpus = 2L); install.packages(c("collapse", "nanoparquet"), repos = supplemental_repository, Ncpus = 2L); expected <- c(IRkernel = Sys.getenv("IRKERNEL_VERSION"), jsonlite = Sys.getenv("JSONLITE_VERSION"), rlang = Sys.getenv("RLANG_VERSION"), tibble = Sys.getenv("TIBBLE_VERSION"), data.table = Sys.getenv("DATA_TABLE_VERSION"), collapse = Sys.getenv("COLLAPSE_VERSION"), nanoparquet = Sys.getenv("NANOPARQUET_VERSION"), arrow = Sys.getenv("ARROW_VERSION"), clock = Sys.getenv("CLOCK_VERSION")); actual <- vapply(names(expected), function(package) as.character(utils::packageVersion(package)), character(1)); stopifnot(as.character(getRversion()) == "4.5.2", identical(actual, expected)); IRkernel::installspec(user = FALSE, prefix = "/opt/openwrangler/venv", name = "openwrangler-r-remote-acceptance", displayname = "R (Open Wrangler Remote)")'
