...
test("packaged Python editor loops stop on the product's terminal diagnostic", async () => {
  ...
  for (const message of [
    ...
    "The Python editor action failed after"
  ]) {
    const messageIndex = source.indexOf(message);
    assert.ok(messageIndex >= 0, `Missing packaged Python terminal diagnostic: ${message}`);
    const guardIndex = source.lastIndexOf('if (pythonDiagnostics.stage === "failed")', messageIndex);
    const loopIndex = source.lastIndexOf("do {", messageIndex);
    assert.ok(
      guardIndex > loopIndex && guardIndex < messageIndex,
      `The packaged Python loop must throw immediately from the terminal diagnostic before ${message}`
    );
  }

  const quartoStart = source.indexOf("async function exerciseReleasedPythonQuartoDocumentJourney(");
  const quartoEnd = source.indexOf("\nasync function assertReleasedNativeREditorTooling(", quartoStart);
  assert.ok(quartoStart >= 0 && quartoEnd > quartoStart, "The packaged Python Quarto journey must exist.");
  const quarto = source.slice(quartoStart, quartoEnd);
  const failedGuard = quarto.indexOf('if (pythonDiagnostics.stage === "failed")');
  const failedCheckpoint = quarto.indexOf(
    '`${checkpoint}:python-${pythonDiagnostics.lastActiveStage ?? "unknown-stage"}:failed`',
    failedGuard
  );
  const boundedDiagnostics = quarto.indexOf(
    "await boundedReleasedJupyterQuickInputDiagnostics(workbench)",
    failedGuard
  );
  const failureThrow = quarto.indexOf("The Python Quarto action reported a terminal failure.", failedGuard);
  assert.ok(
    failedGuard >= 0 &&
      failedCheckpoint > failedGuard &&
      boundedDiagnostics > failedCheckpoint &&
      failureThrow > boundedDiagnostics,
    "The Python Quarto terminal checkpoint must publish before bounded UI diagnostics and the failure throw."
  );
  for (const boundedProbe of [
    /probeAcceptanceBeforeDeadline\(\s*\(\) => releasedPythonFailureNotification\(workbench\),\s*passiveProbeDeadline/u,
    /probeAcceptanceBeforeDeadline\(\s*\(\) => visibleReleasedJupyterConsentCount\(workbench\),\s*passiveProbeDeadline/u,
    /probeAcceptanceBeforeDeadline\(\s*\(\) => visibleReleasedJupyterQuickInput\(workbench\),\s*passiveProbeDeadline/u,
    /probeAcceptanceBeforeDeadline\(\s*\(\) => releasedJupyterQuickPickRow\(picker, target\.label\),\s*passiveProbeDeadline/u,
    /probeAcceptanceBeforeDeadline\(\s*\(\) => releasedJupyterRouteLabel\(picker, target\.routeLabels\),\s*passiveProbeDeadline/u,
    /probeAcceptanceBeforeDeadline\(\(\) => input\.count\(\), passiveProbeDeadline\)/u,
    /probeAcceptanceBeforeDeadline\(\s*\(\) => releasedJupyterQuickPickRow\(picker, label\),\s*passiveProbeDeadline/u
  ]) {
    assert.match(quarto, boundedProbe);
  }
  assert.equal((quarto.match(/probeAcceptanceBeforeDeadline\(/gu) ?? []).length, 7);
  assert.match(quarto, /const passiveProbeDeadline = Date\.now\(\) \+ WORKBENCH_DIAGNOSTIC_TIMEOUT_MS;/u);
  assert.doesNotMatch(quarto, /workbench\.waitForTimeout\(/u);
  assert.match(
    quarto,
    /withBoundedAcceptancePromise\(\s*consent\.dialog\.waitFor\(\{ state: "hidden", timeout: 10_000 \}\)/u
  );
  assert.match(quarto, /withBoundedAcceptancePromise\(\s*closeExactReleasedPythonInteractiveWindow\(interactive\)/u);
  assert.equal(
    (quarto.match(/withBoundedAcceptancePromise\(\s*vscode\.window\.tabGroups\.close\(sourceTab, true\)/gu) ?? [])
      .length,
    2,
    "Both successful and failed Python Quarto source-tab cleanup must be bounded."
  );
  assert.match(quarto, /withBoundedAcceptancePromise\(\s*disposePackagedSessionPanel\(/u);
  assert.match(quarto, /if \(!interactive\) \{\s*const candidates = vscode\.workspace\.notebookDocuments\.filter/u);
  assert.match(source, /did not finish\(\?: within\|\\s\+\(\?:opening\|preparing\)\)/u);
  ...
  for (const name of [
    "dismissReleasedJupyterKernelPicker",
    "waitForReleasedJupyterKernelLabel",
    "visibleReleasedJupyterQuickInput",
    "releasedJupyterQuickInputDiagnostics",
    "visibleReleasedJupyterConsentCount"
  ]) {
    const helper = helperBody(name);
    assert.match(helper, /workbench\s*\.mainFrame\(\)\s*\.locator\(/u, `${name} must query native workbench chrome.`);
    assert.doesNotMatch(
      helper,
      /releasedWorkbenchFrames\(/u,
      `${name} must not scan renderer or auxiliary frames for native workbench chrome.`
    );
  }
  const consentWait = helperBody("waitForReleasedJupyterConsent");
  const consentProbe = consentWait.slice(0, consentWait.indexOf("if (!dialog)"));
  assert.match(consentProbe, /workbench\s*\.mainFrame\(\)\s*\.locator\(/u);
  assert.doesNotMatch(consentProbe, /releasedWorkbenchFrames\(/u);
  const quickPickRow = helperBody("releasedJupyterQuickPickRow");
  assert.match(quickPickRow, /quickInput\.locator\(/u);
  assert.doesNotMatch(quickPickRow, /(?:workbench|releasedWorkbenchFrames)/u);
  const routeLabel = helperBody("releasedJupyterRouteLabel");
  assert.match(routeLabel, /releasedJupyterQuickPickRow\(quickInput, label\)/u);
  assert.doesNotMatch(routeLabel, /(?:workbench|releasedWorkbenchFrames)/u);

  for (const name of [
    "waitForNotebookPreviewConflict",
    "assertNotebookPreviewConflictAbsent",
    "resolveReleasedNotebookEditorTitleAction",
    "resolveReleasedNotebookToolbarAction",
    "releasedNotebookLaunchSurfaceMatches",
    "probeReleasedNotebookToolbarOverflow",
    "releasedNotebookToolbarDiagnostics",
    "waitForReleasedNotebookVariablePicker"
  ]) {
    const helper = helperBody(name);
    assert.match(helper, /workbench\.mainFrame\(\)/u, `${name} must query only native workbench chrome.`);
    assert.doesNotMatch(
      helper,
      /releasedWorkbenchFrames\(/u,
      `${name} must not scan renderer or auxiliary frames for native workbench chrome.`
  ...
  for (const name of [
    ...
    "waitForReleasedJupyterVariableAction",
    "releasedWorkbenchDiagnostics"
  ]) {
    assert.match(
      helperBody(name),
      /releasedWorkbenchFrames\(workbench\)/u,
      `${name} must retain genuine renderer discovery.`
    );
  }
  assert.match(consentWait, /releasedWorkbenchFrames\(workbench\)/u);
});
