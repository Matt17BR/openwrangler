...
const expectedStableRScope = [
  ...
  ["Copy or save generated R", "All 29 catalog operations", "src/test/rCompleteCatalogCodeExport.unit.test.ts"],
  [
...
test("binds the checked-in Native R preview table to its exact truthful structure", () => {
  ...
  const previewOperationScope = R_PREVIEW_PARITY_SCOPE.filter(([surface]) =>
...
test("rejects structural and semantic drift in the stable Native R matrix", () => {
  ...
  for (const mutated of [
    ...["All 28 catalog operations", "All 30 catalog operations"].map((availability) =>
      stableRParity().replace("All 29 catalog operations", availability)
    ),
