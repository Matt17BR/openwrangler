/* Native identity and signaling for the source R contract runner. */
...
struct record {
  pid_t pid;
  struct combined_info info;
  uint64_t before_parent_unique_id;
  uint32_t before_version;
  int marked;
};
...
/* After argc arguments, KERN_PROCARGS2 includes environment and Apple metadata.
 * The bounded exact-marker scan cannot distinguish those two kinds of tail. */
...
static int inspect(pid_t pid, const char *owner, char *buffer, struct record *record, const char **refusal) {
  ...
  /* Exec changes an image, not its lifetime. Retain both samples for ancestry. */
  record->pid = pid; record->info = after; record->marked = marked;
  record->before_version = (uint32_t)before.unique.id_version;
  record->before_parent_unique_id = before.unique.parent_unique_id;
...
static void emit(const struct record *record) {
  ...
  printf("{\"pid\":%d,\"parentPid\":%u,\"startIdentity\":\"%" PRIu64
    ...
    ",\"beforeIdVersion\":%" PRIu32 ",\"beforeParentUniqueId\":\"%" PRIu64
    "\",\"originalParentVersion\":%" PRIu32 ",\"marker\":%d}", record->pid,
    record->info.bsd.pbi_ppid, u->unique_id, u->parent_unique_id,
    (uint32_t)u->id_version, record->before_version, record->before_parent_unique_id,
    (uint32_t)u->original_parent_version, record->marked);
}
...
static int signal_target(struct target target, int requested) {
  ...
  if (found < 0) return 2;
  if ((uint32_t)current.unique.id_version != target.version) return 2;
...
static int preflight(void) {
  ...
  audit_token_t stale_token = {{0}}; stale_token.val[5] = (uint32_t)self.pid; stale_token.val[7] = stale.version;
  if (proc_signal_with_audittoken(&stale_token, SIGCONT) != ESRCH ||
      signal_target(stale, SIGCONT) != 2 || signal_target(self, SIGCONT) != 0) return fail("native token preflight refused");
