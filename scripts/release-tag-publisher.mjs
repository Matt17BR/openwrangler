...
function readVersion(root) {
  ...
  if (Buffer.byteLength(source, "utf8") > PACKAGE_JSON_MAX_BYTES) {
    throw new Error("package.json exceeds the release-tag publication bound.");
  }
  ...
  if (
    ...
    !RELEASE_VERSION.test(manifest.version)
  ) {
    throw new Error("package.json must contain one canonical release version.");
  }
...
function inspectRemoteTag({ expectedCommit, gitRunner, releaseTag, root }) {
  const stdout = git(
    ...
    "inspect the exact public release tag",
    gitRunner
  );
  ...
  if (receipt.annotated) {
    throw new Error("The release tag must be an exact lightweight tag, not an annotated tag.");
  }
  return receipt;
}

function createCredentialLease(token) {
  const root = mkdtempSync(join(tmpdir(), "ow-release-tag-"));
...
function pushTag({ expectedCommit, gitRunner, releaseTag, root, token }) {
  ...
  try {
    ...
    delete env.GH_TOKEN;
    git(root, args, "atomically push the one exact release tag", gitRunner, { env });
  } catch (error) {
    pushError = error;
  }
  try {
    ...
    if (pushError !== undefined) {
      throw new AggregateError([pushError, cleanupError], "Release-tag push and credential cleanup both failed.");
    }
    throw cleanupError;
  }
...
function validateSourceRef(sourceRef) {
  if (
    ...
    !/^refs\/remotes\/origin\/[A-Za-z0-9][A-Za-z0-9._/-]{0,200}$/u.test(sourceRef) ||
    sourceRef.includes("..") ||
    sourceRef.includes("//") ||
    sourceRef.includes("@{") ||
    sourceRef.endsWith("/") ||
    ...
    throw new Error("The release source ref must be one canonical origin remote-tracking ref.");
  }
}
...
function assertRepositoryCheckout({ expectedCommit, gitRunner, root }) {
  ...
  if (discoveredRoot !== root) {
    throw new Error("Release-tag publication must run at the repository root.");
  }
