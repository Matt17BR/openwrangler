...
posixTest("phase-loader staging derives one fixed reachable static ESM closure", () => {
  ...
  try {
    ...
    assert.equal(stage.manifest.directories.length, 1);
    assert.deepEqual(
...
function phaseFixture(prefix) {
  ...
  writeFileSync(join(source, "remote-workspace-phase-child.mjs"), validChildSource());
  writeFileSync(join(source, "remote-workspace-contract.mjs"), "export const contract = 1;\n");
  writeFileSync(join(source, "remote-workspace-processes.mjs"), "export const processes = 1;\n");
